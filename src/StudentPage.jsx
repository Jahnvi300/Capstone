...
export default function StudentPage({ userName, onLogout, addGrievance, grievances, stats }) {
  const [selected, setSelected] = useState("newComplaint");
  const [text, setText] = useState("");
  const [category, setCategory] = useState("General");

  const myList = useMemo(
    () => grievances.filter(g => g.submittedBy === "Student" && (g.submitterName || "").toLowerCase() === (userName || "").toLowerCase()),
    [grievances, userName]
  );

  const submit = () => {
    if (!text.trim()) return;
    addGrievance(text, "Student", userName, category);
    setText("");
    setCategory("General");
  };
  ...
  return (
    <div style={{ display: "flex" }}>
      <Sidebar role="student" selected={selected} setSelected={setSelected} userName={userName} onLogout={onLogout} />
